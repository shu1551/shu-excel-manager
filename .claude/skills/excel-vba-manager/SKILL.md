...
7. **表を直した後の報告は要点だけ 1〜3 行**（何をどこに直したかを番地で。経過・秒数・長い説明は書かない）。独立な調査・実行は 1 往復に並列で畳む。「遅い」はまず `stats` で内訳（道具の中 vs 間）。
...
- **表を直した後の報告は要点だけ 1〜3 行**（2026-09-25 shu「直しましただけで中身が全然ないのはダメ・長い説明はいらない」）。
