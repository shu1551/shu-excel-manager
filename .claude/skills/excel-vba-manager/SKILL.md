...
7. **表を直した後の報告は「直しました」の一言**（中身は聞かれたら）。独立な調査・実行は 1 往復に並列で畳む。「遅い」はまず `stats` で内訳（道具の中 vs 間）。
...
4. **完了条件**（報告の前に自分で確かめる）: 指示の項目が全部当たっている／### 0／見出し・罫線・列幅・
   ...
   保存はしていない。表の見た目を変えたら **`screenshot` で 1 回見てから渡す**
...
- **表を直した後の報告は要らない**（2026-09-24 夜 shu「報告なんて要らねえ」）。終わったら「直しました」の一言だけ。
  直した中身・秒数は聞かれたら番地で答える（報告を書くだけで 9 秒かかっていた）。往復は 4 回が上限。
