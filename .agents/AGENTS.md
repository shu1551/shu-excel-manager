...
1. **道具を直撃する**
   - Excel の話が出たら、最初の手で `excel-manager` の道具を撃つ。表の修正なら `vba("seiri")`、それ以外は `vba("materials")`。
...
2. **表の修正は棚から**
   - `seiri`（表を整えるマクロ＋残りの報告）→ 報告の「棚で直せる手」を `shelf-run` で上から →
     頼みの文があれば `shelf --ask "依頼文"` で棚を引いて `shelf-run` →
...
3. **2 往復で終える**
   - 見る（`seiri`。表の中身が要る「残り」があるときだけ `materials` を足す）→ やる（`shelf-run`・`write-cells`・`tidy` を同じ返事に並べて 1 回で）。
