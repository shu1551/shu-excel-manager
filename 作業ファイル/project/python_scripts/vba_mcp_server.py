...
集計表やグラフを作る・点検の一覧 など）:
 1. vba("seiri 頼みの文") を 1 回（先撃ち）。使う人の頼みの文をそのまま渡すと、表を整えるマクロ・頼みに当たる棚のマクロ・tidy の仕上げまで
    道具が 1 回の呼び出しで撃ち、直したセル（前→後）と残り（エラーの式・気づき）を返す。頼みの語が無い「表の修正」だけなら、
    vba("seiri") を引数なしで撃つ（汚れは道具が見つけて棚で直す）。棚を 1 手ずつ撃たない（手の間ごとに考える分だけ遅くなる）。
 2. 残りが「エラーセル: なし」なら、それで仕上がり＝道具をこれ以上撃たずに 3 の報告を書いて終わる（screenshot・気づきの直し・
    寄せや色の手直しを足さない。気づきは使う人が頼んだときだけ直す）。
    エラーセルがあるときだけ、表と式を見て vba("write-cells C7 値 …") / write_grid と vba("tidy 表の範囲") を同じ返事に並べて 1 回で撃つ。
    マイナス・桁違い・番号の重複・式に直書きの数など、人の判断が要るものは直さずに番地で報告する。
 3. 報告は要点だけ 3 行程度（5 行以内）（何をどこに直したかを番地で。例: C7・C11 の郵便番号をハイフンつきに／F・G 列に突き合わせの式）。
