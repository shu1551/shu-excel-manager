...
def split_into_subs(lines):
    ...
    header = []
    blocks = []
    pending = []        # Sub と Sub の間の行（説明コメント等）。次の Sub の前置きとして残す
    i = 0
    while i < len(lines):
        if SUB_START.match(lines[i].strip()):
            ...
            while i < len(lines):
                ...
                if SUB_END.match(lines[i].strip()):
                    break
                i += 1
            # 2026-09-16: それまで Sub の間の行を黙って捨てていた（テストの説明コメント 10 行が消えた）。
            # 前後の空行だけ落として、その Sub の前置きとして持つ
            pre = list(pending)
            while pre and pre[0].strip() == '':
                pre.pop(0)
            while pre and pre[-1].strip() == '':
                pre.pop()
            blocks.append((sub_lines, start, pre))
            pending = []
            i += 1
        else:
            if not blocks:
            ...
            i += 1
    # 最後の Sub の後ろに残った行は末尾の前置き（blocks の外）として返す
    tail = list(pending)
...
def format_content(content):
    ...
    for i, (sub_lines, _, pre) in enumerate(blocks):
        result.append('')   # Sub 間に1行空行
        if pre:
            result.extend(pre)      # Sub の前の説明コメント等（消さない）
            if sub_lines and SUB_START.match(pre[-1].strip()) is None and pre[-1].strip().startswith("'"):
                pass                # 説明コメントは Sub に密着させる（空行を挟まない）
            elif sub_lines:
                result.append('')
        if not sub_lines:
            continue
