...
def test_column_misfits_catch_what_the_exam_found():
    ...
    rows = [["会社名", "電話", "郵便番号", "品番", "氏名", "登録日", "金額"],
            ...
            ["(株)B", "090 8568 3136", "0100952", "Ｐ－１００２", "鈴木　恵", "2026/1/2", "１２０００"],
    ...
    text = "\n".join(got)
    for want in ("（株）と株式会社", "電話番号の書き方", "B3 '090 8568 3136'", "郵便番号が 000-0000", "C3 '0100952'",
    ...
    clean = [rows[0], ["株式会社A", "03-1234-5678", "010-0951", "P-1001", "佐藤 誠", dt.datetime(2026, 1, 1), 1000.0],
             ["株式会社B", "090-8568-3136", "010-0952", "P-1002", "鈴木 恵", dt.datetime(2026, 1, 2), 12000.0]]
...
def test_normalize_corp_rule():
    ...
    for src, want in (('（株）アクアテック', '株式会社アクアテック'), ('㈱北斗精機', '株式会社北斗精機'),
                      ...
                      ('有限会社　七夕企画', '有限会社七夕企画'), ('(有)白樺ソフト', '有限会社白樺ソフト'),
                      ('こだま製菓(株)', 'こだま製菓株式会社'), ('株式会社双葉精密', '株式会社双葉精密')):
    ...
    class _Sheet:
        class UsedRange:
            Value = (("会社名", "担当者", "電話"), ("株式会社A", "佐藤 誠", "03-1111-2222"),
                     ("株式会社A", "鈴木 恵", "03-3333-4444"), ("株式会社B", "高橋 学", "03-5555-6666"),
                     ("株式会社C", "田中 彩", "03-7777-8888"))
