...
Sub マクロ名()
  ...
  依頼の語は、この仕事を名指しする具体的な語（管理会計・突合・集約など）を入れる。「集計」「まとめ」のような
...
規則（守らないと道具が受け付けない）:
 ...
 - 対象は ActiveSheet の表。表は「見出し 1 行＋本文」（上に表題の行があることもある）。見出しの行と列の範囲は UsedRange から
   ...
   **見出しの行＝値（数・日付）が並ぶ本文のすぐ上の行**。表題・「単位：円」・「出力日 …」・「合計 / …」「列ラベル」の行は
   ...
   見出しの行も同じ決め方で探す。
 - **マクロは現場のどんな表にも撃たれる。見出しの語・シート名・テーブル名・課や科目や人の名前を書き込まない。**
   道具は見出しの語とシート名を変えた別の表（課名→所属・部署名、支出額→金額・実績額、支出明細→明細・Sheet1 など）でも試し、
 ...
 - ブックは ActiveWorkbook（または ActiveSheet.Parent）。ThisWorkbook は使わない（マクロはアドインに入る＝ThisWorkbook は
   表のブックではない）。CreateObject は "Scripting.Dictionary" だけ（System.Collections.* は現場の PC で動かない）。
...
def harvest_pair(name, before_path, truth_path, sheet=None, request='', tests=()):
    ...
    現場の作業は、汚れた表と人が直した表の 2 冊があれば鍛えられる。AI の手の記録は無い＝差がすべて。"""
...
def _prompt(case, feedback=None, prev_code=None):
    ...
    if case.get('ref_code') and not prev_code:
        parts.append("\n--- 前の版（表の語を書き込んでいたので、見出しの語が違う現場の表で動かなかった。オブジェクトの作り方・"
...
def _validate_code(code):
    ...
    if bad:
        return None, f"CreateObject は \"Scripting.Dictionary\" だけ（{bad[0]} は現場の PC で動かないことがある）", None
    why = _too_long(code)
    if why:
...
_GENERIC_REQUESTS = ('表をきれいに整えてください', '同じ行が二重になっているので消して', '日付の順に並べ替えて',
                     ...
                     '達成率の列を足して', '実績額の列を太字にして', '予算額を千円単位にして', '見出しのセルをマージして中央に',
...
def _literal_words(case, code):
    ...
    2026-09-18: 鍛えた 27 本が課名・支出額・T支出明細・課マスタを書き込み、見出しの語が違う現場の表で動かなかった
...
def _literal_error(case, code):
    ...
    if not words:
        return None
    return ("表の語をマクロに書き込んでいます（現場の表は見出しの語・シート名・テーブル名・項目の名前が違う＝その表では動かない）: "
...
def _check_fit(case, code, ask_re):
    ...
    for other, words in _OTHER_JOB_WORDS.items():
        ...
        if other != mine and heads and all(vp.head_in(h, words) for h in heads):
            return None, (f"' 見出し: の語（{'・'.join(heads)}）が、ほかの仕事「{other}」の表にもそろっています＝その表でも撃たれます。"
                          "この表にしか無い見出しの語を足す（例: 月の列の「4月」・「管理会計区分」など）")
    ...
    if probs:
        return None, ("' 依頼の語: の決まり違反（全部を同時に満たす）: " + " ／ ".join(f"{i}) {p}" for i, p in enumerate(probs, 1))
                      + f"。語は 2〜{_ASK_WORD_MAX} 字・この仕事にしか出ない語（例: 突合・新システム・集約・按分・管理会計・受付簿）を | で並べる")
...
def _pick_register_target(books, to=None):
    ...
    → (ブック名, None) か (None, 理由)。
    2026-09-17: 登録先を「アクティブなブック」にしていたので、秀コンボがアクティブだと秀コンボへ、現場のブックが
    アクティブだと現場のブックへ入った（先撃ちが探すのは「表を整える」の持ち主）。名指し か 持ち主 1 冊 に限る。"""
