...
#   システムの移行の仕事は、どれも「旧システムの出力を隣に置いて照らす」形。
...
def _read_text_decode(raw):
    """CSV の文字コードを当てる → (文字列, 名前)。BOM → utf-8 → cp932 の順（古い CSV は cp932 が多い）。"""
...
# ----------------------------------------------------------------
# 電話番号の区切り（2026-09-11: AI が正規表現で区切りを当て推量し、名古屋の 052-333-4444 を 05-2333-4444 にした）。
