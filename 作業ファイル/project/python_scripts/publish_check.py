...
2026-07-11 の実名メタデータ公開事故を受けた機械チェック。見る先:
  ...
  - 全パーツのバイト列  合言葉・中継URL・APIキー（フォームの .frx に置いた値の残骸を含む）
...
使い方:
  ...
  py publish_check.py <ブック.xlsm> --secret 合言葉  # この文字列が中に無いことを確かめる（複数可）
...
# 見つけたら無条件で公開を止める文字列の形。誤検知でゲートが鳴りっぱなしになると
# exit 1 を無視する癖が育つので、当たれば確実に秘密だと言える形だけを並べる。
SECRET_PATTERNS = [
    ('GAS中継URL', r'https://script\.google\.com/macros/s/[A-Za-z0-9_\-]{20,}'),
    ('Google APIキー', r'AIza[0-9A-Za-z_\-]{35}'),
    ('Anthropic APIキー', r'sk-ant-[A-Za-z0-9_\-]{20,}'),
    ('OpenAI APIキー', r'sk-[A-Za-z0-9]{32,}'),
    ('GitHubトークン', r'gh[pousr]_[A-Za-z0-9]{20,}'),
    ('Slackトークン', r'xox[baprs]-[A-Za-z0-9\-]{10,}'),
]


def _mask(v):
    """秘密そのものを画面に出さない（頭4文字と長さだけ）"""
    return f'{v[:4]}…[{len(v)}文字]'


def _decode_all(data):
    """ブックの中の文字列は CP932・UTF-16LE・UTF-8 のどれでもありうる。
    フォームの設計時プロパティ（.frx）は UTF-16LE、モジュールのソースは CP932。"""
    for enc in ('cp932', 'utf-16-le', 'utf-8'):
        yield enc, data.decode(enc, 'ignore')


def find_secrets(z, secrets):
    """合言葉・APIキー等が中に残っていないかを全パーツのバイト列で探す。

    値を消して保存し直しても、フォームの .frx やモジュールのソースの断片が
    vbaProject.bin のバイト列に残ることがある（2026-08-25 実測：削除したプロシージャの
    ソースが CP932 で、上書き前の .frx の中身が UTF-16LE で残っていた）。
    「画面上の値を消した」は消えた証拠にならないので、現物のバイト列で確かめる。
    """
    named = set()      # (パーツ, マスクした値)
    patterned = set()  # (パーツ, 種別, マスクした値)
    for n in z.namelist():
        try:
            data = z.read(n)
        except Exception:
        ...
        if not data:
            continue
        for _enc, text in _decode_all(data):
            for s in secrets:
                if s and s in text:
                    named.add((n, _mask(s)))
            for label, pat in SECRET_PATTERNS:
                for m in re.findall(pat, text):
                    patterned.add((n, label, _mask(m)))
    return sorted(named), sorted(patterned)


def check(path, secrets=()):
    ...
    # 秘密の文字列（明示指定＋高精度パターン）。フォームの .frx に置いた設定は
    # シートに出ないので目視では見つからない。残骸は「値を消して保存」では落ちず、
    # 新しい器へ引っ越す（秀なら shu003「マクロの引っ越し」）と消える。
    named, patterned = find_secrets(z, secrets)
    print(f'  秘密の文字列: 指定 {len(named)}件 / パターン {len(patterned)}件')
    for n, v in named:
        print(f'      ★{n}: {v}')
    for n, label, v in patterned:
        print(f'      ★{n}: {label} {v}')
    if named:
        issues.append(f'指定した秘密の文字列が中に残っています: {named}'
                      '（画面の値を消しても .frx やソースの断片として残ります。'
                      '新しい器へ引っ越して作り直すこと）')
    if patterned:
        issues.append(f'APIキー・中継URL等が中にあります: {patterned}')
...
def main():
    ...
    ap.add_argument('--secret', action='append', default=[], metavar='文字列',
                    help='この文字列が中に無いことを確かめる（合言葉・キー等。複数回指定可）')
    args = ap.parse_args()
    ng = 0
    for p in args.files:
        print(f'===== {p}')
