# -*- coding: utf-8 -*-
"""選んでいる 2 列で構成比のピボットを作る（どの表でも動く形・2026-09-18 作り直し。前の版は科目別構成比ピボット）。

列の決まり: 選んでいる 2 列（1 つ目＝項目、2 つ目＝値）。選んでいる列は <名前>_選択.txt に書く。
正解の決まり:
  - 新しいシート（名前は 項目の列の見出し＋「別構成比」）の A3 にピボット。同じ名前のシートがあれば消して作り直す。
  - 行＝項目の列。値は 2 つ目の列の合計（表示形式 #,##0）と、同じ列の「列集計に対する比率」（表示形式 0.0%）の 2 つ。
  - 項目は合計の大きい順に並べる。総計あり。
  - 元の範囲は明細の見出しの行から最後の明細の行まで（下の合計の行は入れない）。
組: 本番 60 行／A 1,500 行／B 表題と単位の行／C 前の結果のシート／D 明細の下に合計の行／E 列の並びが違う／
  F 撃った後にもう一度／G 空行と番号の列
"""
import os
import random
import shutil
...
REQUEST = ('選んでいる 2 列（1 つ目を項目、2 つ目を値）で、新しいシートに構成比のピボットテーブルを作ってください。'
           '新しいシートの名前は 項目の列の見出しと「別構成比」をつないだ名前。その A3 にピボットを置く。'
           '行は項目の列、値は 2 つ目の列の合計（表示形式 #,##0）と、同じ列の列集計に対する比率（表示形式 0.0%）の 2 つ。'
           '項目は合計の大きい順に並べ、総計あり。'
           '元の範囲は明細の見出しの行から最後の明細の行まで（下の合計の行は入れない）。'
           '同じ名前のシートが既にあれば消して作り直す')
PHRASES = ['選んだ列で構成比のピボットを', '割合つきのピボットを作って', '構成比のピボットにして',
           '比率も出すピボットを作って', '大きい順に構成比のピボット', '選んでいる列で割合のピボット']


def out_name(t):
    return f"{detail.name_of(t, 'kamoku')}別構成比"
...
def build_pivot(wb, t, lay, last=None):
    ...
    item = detail.name_of(t, 'kamoku')
    f = pt.PivotFields(item)
    f.Orientation = 1
    df = pt.AddDataField(pt.PivotFields(num), '合計 / ' + num, -4157)
    df.NumberFormat = '#,##0'
    df2 = pt.AddDataField(pt.PivotFields(num), '構成比', -4157)
    df2.Calculation = 7                      # xlPercentOfColumn
    df2.NumberFormat = '0.0%'
    f.AutoSort(2, '合計 / ' + num)           # 2＝xlDescending（大きい順）
    pt.RowGrand = True
    pt.ColumnGrand = True
    return pt
...
def make(jobs, out):
    ...
    try:
        ...
        for stem, t, kw in jobs:
            ...
            sel = ",".join(f"{get_column_letter(detail.col_of(t, role))}{lay['h']}" for role in ('kamoku', 'num'))
            if kw.get('stale'):
...
def main():
    if len(sys.argv) > 2 and sys.argv[1] == '--unseen':
        ...
        return
    make([('構成比_本番', detail.design(3, 60), {}),
          ('構成比_試験A', detail.design(13, 1500, n_items=10), {}),
          ('構成比_試験B', detail.design(23, 60, title=True, unit=True), {}),
          ('構成比_試験C', detail.design(33, 50), {'stale': True}),
          ('構成比_試験D', detail.design(43, 40, total=True), {}),
          ('構成比_試験E', detail.design(53, 45, swap=True), {}),
          ('構成比_試験G', detail.design(63, 35, blanks=2, no_col=True), {})], HERE)
    for x in ('前', '正解'):
