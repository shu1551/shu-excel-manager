...
- 特定ブック名・パス・モジュール名に**限定／依存させない**。
- 既存コードに固有名詞（ブック名・モジュール名）があっても「対象」と鵜呑みにしない。
  **固有名詞より「対象はアクティブブック」を先に当てる**。

## 作業の前提

- MCP サーバー `excel-manager` が登録されていれば、道具は MCP が第一選択（`mcp__excel-manager__vba("コマンド1行")` ほか）
- スクリプトは `作業ファイル/project/python_scripts/` から実行する
- Python は `py` コマンドを使う（`python` ではない）
- 操作したいブックを Excel で開いてアクティブにしておく
...
**「動かない」の相談は `repair <マクロ名>` 一発**——本文・呼び元・コンパイルの落ちた行・検査の結果が 1 画面に出る。
同じ返答の中で仮説まで言い切る。仮説は一言で会話に出し、直し方（最小の一手か根本直しか）は
利用者が選ぶ。利用者がその場にいるなら検証は本人の実機テストが最速。
